...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    initListener()
  }, []) // التعديل هنا: القوس الفارغ يمنع التكرار اللانهائي

  // أمان إضافي: إيقاف التحميل بعد 3 ثوانٍ في حال تأخر البيانات أو كانت القائمة فارغة
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const product = products && products.length > 0 ? products.find((p) => p.id === params.id) : null
    ...
    if (product) {
      ...
      if (product.sizes && typeof product.sizes === 'object') {
        ...
        const sizesSource = product.sizes;
        ...
        sizeOrder.forEach(key => {
            ...
            if (sizeDetails && sizeDetails.price && Number(sizeDetails.price) > 0) {
                foodItemSizes.push({
        ...
        // نقوم بإضافة الأحجام الأخرى التي لم تكن في القائمة المرتبة
            ...
            if (!alreadyAdded && val && val.price && Number(val.price) > 0) {
                foodItemSizes.push({
                    name: key as Size["name"],
        ...
        foodItem = {
          ...
          featured: false,
          roastLevel: "وسط",
          sizes: [{ name: "100g", price: Number(product.price || 0), images: [product.image || "/placeholder.svg"] }],
        };
      }
      ...
      // إذا تم تحميل المنتجات (القائمة ليست فارغة) ولكن هذا المنتج غير موجود
      setLoading(false)
    }
  }, [params.id, products])
