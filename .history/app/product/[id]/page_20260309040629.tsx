  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setSelectedSize((prev) => {
        if (prev) {
          const sameSizeInNewData = foodItem.sizes.find((s) => s.name === prev.name)
          if (sameSizeInNewData) return sameSizeInNewData
        }
        return foodItem.sizes.find((s) => s.name === "100g") || foodItem.sizes[0] || null
      })
    }
    setLoading(false)
  }, [params.id, products])
  ...
  const handleAddToCart = () => {
    if (item && selectedSize && addFunction) {
      ...
      addFunction({
        ...
        image: selectedSize.images[0], // استخدام صورة الحجم المختار كصورة مصغرة
        quantity: 1,
