...
export default function AdminLogin() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                // تصفية المستخدمين المحذوفين (Soft Delete)
    ...
    const handleDeleteUser = async (e: React.MouseEvent, userId: string, userName: string) => {
        e.stopPropagation();
        if (window.confirm(`هل أنت متأكد من حذف حساب ${userName}؟\nلن يتمكن من الدخول مجدداً، ولكن ستبقى إحصائيات مبيعاته محفوظة.`)) {
            try {
                // Soft Delete: تحديث الحقل بدلاً من الحذف النهائي
