...
import { Coffee, Package, Users, ShoppingCart, BarChart3, History, Trash2, Edit, Save, X, Search, RotateCcw, Calendar, ArrowUpDown, LogOut, ShieldAlert, ChevronDown, PieChart as PieChartIcon, Key } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  const { isAccessGranted, logout: logoutAdmin, changePassword } = useAdminStore()
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPasswordMsg({ type: 'error', text: error.message || 'حدث خطأ غير معروف.' });
    }
        ...
        <>
          ...
          {activeTab === 'settings' && isAccessGranted && (
            <div className="admin-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2 text-[var(--admin-text)]">
                <Key className="text-blue-500" /> تغيير كلمة مرور المسؤول
              </h3>
              <form onSubmit={handlePasswordChange} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold mb-2 text-[var(--admin-text)]">كلمة المرور الجديدة</label>
                  <input 
                    type="password" 
                    value={newPassword}
                    onChange={(e) => setNewPassword(e.target.value)}
                    className="w-full p-3 rounded-lg border admin-border bg-white/50 dark:bg-gray-800 outline-none focus:ring-2 focus:ring-blue-500 text-[var(--admin-text)]"
                    placeholder="أدخل كلمة المرور الجديدة"
                    required
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2 text-[var(--admin-text)]">تأكيد كلمة المرور</label>
                  <input 
                    type="password" 
                ...
                {passwordMsg.text && (
                  <div className={`p-4 rounded-lg font-bold text-sm ${passwordMsg.type === 'error' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'}`}>
                    {passwordMsg.text}
                  </div>
                )}
                ...
                >
                  {isUpdatingPassword ? 'جاري التحديث...' : 'حفظ كلمة المرور الجديدة'}
                </button>
              </form>
            </div>
          )}
