...
export default function AdminDashboard() {
  ...
  // تم جعل القيم الافتراضية تسمح بالدخول مباشرة لتخطي شاشة التحميل
  const [loadingAuth, setLoadingAuth] = useState(false)
  const [isAuthenticated, setIsAuthenticated] = useState(true)
  const { isAccessGranted, logout: logoutAdmin } = useAdminStore()

  useEffect(() => {
    // تم إيقاف التحقق من Firebase مؤقتاً لكي تظهر الصفحة وتتمكن من العمل عليها
  }, [router])
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth)
      logoutAdmin() // Reset admin state on logout
  ...
  if (loadingAuth) {
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-[70vh] py-20" style={{ backgroundColor: 'var(--admin-bg)' }}>
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center w-full text-center p-8 min-h-[70vh] py-20" style={{ backgroundColor: 'var(--admin-bg)' }}>
        <ShieldAlert size={64} className="text-red-500 mb-4 mx-auto" />
        <h2 className="text-2xl font-bold mb-2 text-[var(--admin-text)]">غير مصرح بالدخول</h2>
        <p className="text-gray-500 mb-6">يجب تسجيل الدخول كمسؤول أولاً. جاري تحويلك...</p>
        <button onClick={() => router.push('/admin-login')} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition">
          الذهاب لصفحة تسجيل الدخول يدوياً
        </button>
      </div>
    )
  }
