...
import { auth, db } from "../../config/firebase" // استيراد db
import { doc, getDoc } from "firebase/firestore" // استيراد دوال Firestore
import { Coffee, Package, Users, ShoppingCart, BarChart3, History, Trash2, Edit, Save, X, Search, RotateCcw, Calendar, ArrowUpDown, LogOut, ShieldCheck, UserCog } from "lucide-react"
...
// Add statusHistory to the Order type
...
export default function AdminDashboard() {
  ...
  const [userProfile, setUserProfile] = useState<UserProfile | null>(null)
    const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setIsLoading(true);
        try {
            // توليد بريد إلكتروني تلقائي في الخلفية (مخفي عن المستخدم)
  ...
  // دالة موحدة للتحقق من المصادقة وجلب ملف المستخدم
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (!user) {
        ...
        if (userDoc.exists()) {
          ...
          // إذا كان الحساب محذوفاً، قم بتسجيل الخروج فوراً
          ...
          // إذا كان المستخدم "staff"، يتم توجيهه إلى صفحة الطلبات فقط
          if (profileData.role === 'staff') {
            ...
            console.error(err);
            setError("حدث خطأ أثناء إنشاء الحساب. تأكد من أن كلمة المرور 6 أحرف على الأقل.");
        } finally {
            setIsLoading(false);
        }
  ...
  const staffSales = historyOrders.reduce((acc, order) => {
    ...
    // البحث عن آخر شخص قام بتحويل الطلب إلى "مكتمل"
  ...
  return (
    <div className="admin-layout flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-8 space-y-8">
        <div className="flex justify-between items-center">
          ...
          {userProfile && (
            <div className="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800 p-2 rounded-lg">
              <div className={`p-1.5 rounded-full ${userProfile.role === 'admin' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                {userProfile.role === 'admin' ? <ShieldCheck size={20} /> : <UserCog size={20} />}
              </div>
              <div>
                <p className="font-bold text-sm text-[var(--admin-text)]">{userProfile.name}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 capitalize">{userProfile.role === 'admin' ? 'مشرف' : 'موظف'}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {activeTab === 'dashboard' && userProfile?.role === 'admin' && (
          <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-zinc-950 p-4" dir="rtl">
            ...
            <div className="admin-card mb-8">
              <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                  <h3 className="text-xl font-bold flex items-center gap-2 text-[var(--admin-text)]"><Users size={20}/> أداء الموظفين (المبيعات)</h3>
                  ...
                  <tbody>
                    {staffSalesList.length > 0 ? staffSalesList.map((staff, index) => (
                      <tr key={index} className="border-b hover:bg-black/5 dark:hover:bg-white/5 transition">
                        <td className="py-4 px-4 text-[var(--admin-text)] font-medium flex items-center gap-2">
                           <div className="w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-xs font-bold">
                             {staff.name.charAt(0)}
                           </div>
                           {staff.name}
                        </td>
                        <td className="py-4 px-4 text-[var(--admin-text)]">{staff.count}</td>
                        <td className="py-4 px-4 text-[var(--admin-text)] font-bold text-green-600">{staff.total.toLocaleString('ar-EG', { style: 'currency', currency: 'EGP' })}</td>
                      </tr>
                    )) : (
                      <tr>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
              ...
              <div className="flex flex-col md:flex-row items-center gap-4 flex-wrap justify-end">
                ...
                <div className="text-center text-sm pt-4">
                    <Link href="/admin-login" className="text-blue-600 hover:underline font-medium flex items-center justify-center gap-1">
                        لديك حساب بالفعل؟ تسجيل الدخول <ArrowRight className="w-4 h-4" />
                    </Link>
                </div>
                      ...
                      <div className="flex flex-col">
                        <select 
                          ...
                          onChange={(e) => {
                            if (updateOrderStatus && userProfile) {
                              updateOrderStatus(order.id, e.target.value as OrderStatus, { uid: userProfile.uid, name: userProfile.name });
                            }
                          }}
                        ...
                        {order.statusHistory && order.statusHistory.length > 0 && (
                          <span className="text-xs text-gray-400 mt-1">
                            بواسطة: {order.statusHistory[order.statusHistory.length - 1].updatedBy.name}
                          </span>
                        )}
                      </div>
                    </td>
        ...
        {activeTab === 'customers' && userProfile?.role === 'admin' && (
          <div className="admin-card overflow-x-auto">
        ...
        {activeTab === 'products' && userProfile?.role === 'admin' && (
                      ...
                      <td className="py-4 px-4 flex gap-2">
                        {editingId === product.id ? (
                          ...
                          <>
                            ...
                            <button 
                              onClick={() => { if (window.confirm('هل أنت متأكد من حذف هذا المنتج نهائياً؟')) { removeProduct(product.id) } }}
