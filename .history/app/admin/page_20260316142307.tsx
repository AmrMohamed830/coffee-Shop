...
import { Coffee, Package, Users, ShoppingCart, BarChart3, History, Trash2, Edit, Save, X, Search, RotateCcw, Calendar, ArrowUpDown, LogOut, ShieldAlert, PieChart as PieChartIcon, ChevronDown } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  // ألوان متناسقة للرسم الدائري
  const COLORS = ['#f97316', '#3b82f6', '#10b981', '#a855f7', '#ef4444', '#eab308'];

  // بيانات الرسم البياني الدائري (أكثر 5 منتجات مبيعاً)
  const topProductsData = Object.entries(productSalesCount)
    ...
    .slice(0, 5); // عرض أعلى 5 منتجات فقط عشان الشكل ميبقاش زحمة
  ...
  if (salesChartRange === '7days') {
    ...
    // عرض أشهر السنة الحالية من يناير إلى ديسمبر
                ...
                <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedPeriod(null)}>
                  <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-4xl p-6 shadow-2xl relative animate-in zoom-in-95" onClick={e => e.stopPropagation()} dir="rtl">
                    ...
                    {(() => {
                      ...
                      const periodPieData = periodTopProducts.slice(0, 5).map((p, index) => ({
                        ...p,
                        color: COLORS[index % COLORS.length]
                      }));
                      ...
                      return (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* القائمة النصية */}
                          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 max-h-[300px] overflow-y-auto border border-gray-100 dark:border-gray-700 custom-scrollbar">
                          ...
                          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center">
                            <h5 className="text-sm font-bold text-gray-500 mb-2">توزيع أعلى 5 منتجات مبيعاً</h5>
                            <div className="h-[250px] w-full" dir="ltr">
                              <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                  <Pie data={periodPieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={5} dataKey="value">
