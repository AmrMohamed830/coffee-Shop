...
export default function AdminDashboard() {
  ...
  // بيانات تجريبية تظهر في حالة تعطل أو فراغ قاعدة البيانات
  const dummyOrders = [
    { id: 'ord_101', status: 'جديد', total: 450, createdAt: new Date().toISOString(), customer: { firstName: 'أحمد', lastName: 'محمد', phone: '01012345678', address: 'القاهرة', notes: 'بدون سكر' }, items: [{ name: 'بن يمني', quantity: 2 }] },
    { id: 'ord_102', status: 'مكتمل', total: 300, createdAt: new Date(Date.now() - 86400000).toISOString(), completedAt: new Date().toISOString(), customer: { firstName: 'سارة', lastName: 'محمود', phone: '01198765432', address: 'الجيزة', notes: '' }, items: [{ name: 'إسبريسو ميكس', quantity: 1 }] }
  ]
  const dummyProducts = [
    { id: 'p1', name: 'بن كولومبي فاخر', price: 250, category: 'قهوة مختصة', image: '/placeholder.svg' },
    { id: 'p2', name: 'إسبريسو ميكس', price: 300, category: 'إسبريسو', image: '/placeholder.svg' }
  ]
  const dummyCustomers = [
    { firstName: 'أحمد', lastName: 'محمد', phone: '01012345678', address: 'القاهرة', notes: 'عميل مميز' },
    { firstName: 'سارة', lastName: 'محمود', phone: '01198765432', address: 'الجيزة', notes: '' }
  ]

  // حماية صارمة: التأكد من أن البيانات القادمة من قاعدة البيانات صالحة فعلياً وليست مصفوفة تالفة
  const hasValidOrders = storeOrders && storeOrders.length > 0 && storeOrders[0]?.id;
  const hasValidProducts = storeProducts && storeProducts.length > 0 && storeProducts[0]?.id;
  const hasValidCustomers = customers && customers.length > 0 && customers[0]?.firstName;

  const orders = hasValidOrders ? storeOrders : dummyOrders as unknown as Order[];
  const products = hasValidProducts ? storeProducts : dummyProducts as any;
  const uniqueCustomers = hasValidCustomers ? customers : dummyCustomers as any;
  ...
  const dailySales = orders.reduce((acc, order) => {
    if (!order || order.status !== 'مكتمل') return acc
    // @ts-ignore
