...
export default function AdminDashboard() {
  ...
  const { orders, updateOrderStatus, products, removeProduct, updateProduct, clearProducts } = useOrderStore()
            ...
            <div className="admin-card overflow-x-auto">
              <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <button 
                    onClick={() => {
                      if (window.confirm('هل أنت متأكد من حذف جميع المنتجات نهائياً؟')) {
                        clearProducts()
                      }
                    }}
                    className="px-3 py-1 text-xs font-bold text-red-600 bg-red-100 rounded hover:bg-red-200 transition"
                  >
                    حذف الكل
                  </button>
                </div>
