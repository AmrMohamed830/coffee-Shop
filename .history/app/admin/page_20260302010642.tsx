...
import { Coffee, Package, Users, ShoppingCart, BarChart3, History, Trash2 } from "lucide-react"
import { AddProductForm } from "@/components/add-product-form"
import { useOrderStore } from "@/lib/orderStore"

export default function AdminDashboard() {
  // @ts-ignore - تأكد من وجود دالة updateOrderStatus في الـ Store
  const { orders, updateOrderStatus, products, removeProduct } = useOrderStore()
      ...
      <main className="flex-1 p-8 space-y-8">
        ...
        {activeTab === 'products' && (
          <div className="space-y-8">
            ...
            <div className="admin-card overflow-x-auto">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><Package size={20}/> قائمة المنتجات</h3>
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b opacity-50">
                ...
                <tbody>
                  ...
                  {products && products.length > 0 ? products.map((product) => (
                    <tr key={product.id} className="border-b hover:bg-black/5 dark:hover:bg-white/5 transition">
                      ...
                      <td className="py-4 px-4">{Number(product.price).toLocaleString('ar-EG', { style: 'currency', currency: 'EGP' })}</td>
                    ...
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-muted-foreground">لا توجد منتجات مضافة</td>
                    </tr>
                  )}
                </tbody>
              </table>
