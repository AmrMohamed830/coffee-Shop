...
import { Coffee, Package, Users, ShoppingCart, BarChart3, History, Trash2, Edit, Save, X, Search, RotateCcw, Calendar, ArrowUpDown, LogOut, ShieldAlert, PieChart as PieChartIcon } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  const dailySales = historyOrders.reduce((acc, order) => {
    // @ts-ignore
  ...
  Object.entries(productSalesCount).forEach(([name, count]) => {
    if (count > topProductCount) { topProductCount = count; topProduct = name; }
  });
  ...
  const last7DaysSales = Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      name: d.toLocaleDateString('ar-EG', { weekday: 'short' }), // عرض اسم اليوم
  ...
  // بيانات الرسم البياني الدائري (حالات الطلبات)
  const orderStatusData = statusOptions.map(option => {
    const count = orders.filter(o => o && o.status === option.value).length
    let hexColor = '#9ca3af'
    if (option.value === 'جديد') hexColor = '#3b82f6'
    else if (option.value === 'قيد التنفيذ') hexColor = '#f97316'
    else if (option.value === 'جاري الشحن') hexColor = '#a855f7'
    else if (option.value === 'مكتمل') hexColor = '#22c55e'

    return { name: option.label, value: count, color: hexColor }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="admin-card flex items-center gap-5 p-6 border-l-4 border-blue-500">
                  ...
                  <div>
                    ...
                    <h3 className="text-4xl font-black text-[var(--admin-text)]">{activeOrders.length}</h3>
                  </div>
                </div>
                <div className="admin-card flex items-center gap-5 p-6 border-l-4 border-green-500">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-1">مبيعات اليوم</p>
                    <h3 className="text-4xl font-black text-[var(--admin-text)]">{dailySales.toLocaleString('ar-EG', { style: 'currency', currency: 'EGP' })}</h3>
                  </div>
                </div>
                <div className="admin-card flex items-center gap-5 p-6 border-l-4 border-purple-500">
                  ...
                  <div>
                    ...
                    <h3 className="text-4xl font-black text-[var(--admin-text)]">{uniqueCustomers.length}</h3>
                  </div>
                </div>
                <div className="admin-card flex items-center gap-5 p-6 border-l-4 border-orange-500 overflow-hidden">
                  <div className="p-4 bg-orange-100 rounded-2xl text-orange-600 shrink-0"><Package size={32} /></div>
                  <div className="overflow-hidden w-full">
                    <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-1">الأكثر مبيعاً</p>
                    <h3 className="text-xl lg:text-2xl font-black text-[var(--admin-text)] truncate w-full" title={topProduct}>{topProduct}</h3>
                    {topProductCount > 0 && <span className="text-xs text-orange-600 font-bold bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded-md mt-1 inline-block">بمبيعات {topProductCount} قطعة</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="admin-card p-6">
                  <h3 className="text-xl font-bold text-[var(--admin-text)] mb-6 flex items-center gap-2">
                    <BarChart3 className="text-blue-500" /> مبيعات آخر 7 أيام
                  </h3>
                ...
                <div className="admin-card p-6">
                  <h3 className="text-xl font-bold text-[var(--admin-text)] mb-6 flex items-center gap-2">
                    <PieChartIcon className="text-purple-500" /> توزيع حالات الطلبات
                  </h3>
                  <div className="h-[300px] w-full" dir="ltr">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} formatter={(value: number) => [`${value} طلب`, 'العدد']} />
