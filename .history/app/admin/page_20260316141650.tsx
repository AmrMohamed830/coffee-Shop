...
export default function AdminDashboard() {
  ...
  let salesChartData: { name: string, total: number }[] = [];
  ...
  if (salesChartRange === '7days') {
    salesChartData = Array.from({ length: 7 }).map((_, i) => {
      ...
      return { name: d.toLocaleDateString('ar-EG', { weekday: 'short' }), total: dayOrders.reduce((sum, order) => sum + (order?.total || 0), 0) }
    })
    ...
    salesChartData = Array.from({ length: 4 }).map((_, i) => {
      ...
      return { name: `أسبوع ${i + 1}`, total: weekOrders.reduce((sum, order) => sum + (order?.total || 0), 0) }
    })
    ...
    salesChartData = Array.from({ length: 12 }).map((_, i) => {
      ...
      return { name: d.toLocaleDateString('ar-EG', { month: 'short' }), total: monthOrders.reduce((sum, order) => sum + (order?.total || 0), 0) }
    })
  }
