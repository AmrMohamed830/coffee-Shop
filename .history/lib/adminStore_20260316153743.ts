...
export const useAdminStore = create<AdminState>((set, get) => ({
  ...
  handlePasswordSubmit: async (password: string) => {
    ...
    // تخطي (Bypass) لتفعيل وضع الأدمن للتجربة بدون الحاجة لـ Firebase
    if (password === "admin") {
  ...
  changePassword: async (newPassword: string) => {
    ...
    // محاكاة النجاح إذا كنت في وضع التجربة (الباسورد admin)
    if (isAccessGranted && !currentUser) {
      return new Promise((resolve) => setTimeout(resolve, 1000)); // محاكاة تحميل لمدة ثانية واحدة
    }
