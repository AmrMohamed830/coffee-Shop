...
export function EditProductForm({ product, isOpen, onClose }: EditProductFormProps) {
  ...
  useEffect(() => {
    ...
    // 1. التعامل مع الهيكل الجديد للأوزان (مصفوفة)
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex justify-center items-center z-50 p-6">
      <div className="admin-card w-full max-w-4xl max-h-[90vh] overflow-y-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          {/* ... [تم إخفاء التفاصيل الأساسية اختصاراً للمساحة، وهي مطابقة لفورم الإضافة] ... */}
            ...
            {sizes.map((size) => (
              <div key={size.id} className="p-4 border rounded-lg admin-border space-y-4 bg-white/20 dark:bg-white/5 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-bold mb-1 text-[var(--admin-text)]">الوزن الحالي</label>
                    <input type="text" value={size.name} onChange={(e) => handleSizeChange(size.id, 'name', e.target.value)} className="w-full p-2 rounded-md border admin-border text-[var(--admin-text)] bg-white dark:bg-gray-800" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold mb-1 text-[var(--admin-text)]">تعديل السعر (ج.م)</label>
                    <input type="number" value={size.price === 0 ? '' : size.price} onChange={(e) => handleSizeChange(size.id, 'price', e.target.value)} className="w-full p-2 rounded-md border admin-border text-[var(--admin-text)] bg-white dark:bg-gray-800" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold mb-1 text-[var(--admin-text)]">تعديل الصورة</label>
                    {/* ... [كود الصورة مطابق لفورم الإضافة] ... */}
                  </div>
                </div>
              </div>
            ))}
