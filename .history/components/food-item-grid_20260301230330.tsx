...
export function FoodItemGrid({ category = null, featured = false, limit }: FoodItemGridProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // إخفاء المنتجات الافتراضية وعرض فقط ما تم إضافته عن طريق لوحة التحكم (التي يبدأ معرفها بـ food_)
      filteredItems = filteredItems.filter((item) => String(item.id).startsWith("food_"))
