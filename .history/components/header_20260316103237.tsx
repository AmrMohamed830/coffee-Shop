...
export function Header() {
  ...
  const { userProfile, isAccessGranted, toggleAdminMode, logout } = useAdminStore()
  ...
  // إظهار أيقونة الدرع دائماً حتى تتمكن من تفعيل الإدارة
  const showAdminButton = true

  const handleAdminClick = () => {
    if (isAccessGranted) {
      logout() // قفل وضع الإدارة والخروج
      router.push('/')
    } else {
      toggleAdminMode() // فتح نافذة الباسورد لتشغيل الإدارة
    }
  }
        ...
        <div className="flex items-center gap-2">
          {showAdminButton && (
             <Button variant="outline" size="icon" onClick={handleAdminClick} title="Admin Mode">
                <Shield className={cn("h-5 w-5", isAdminMode ? "text-green-500" : "text-destructive")} />
             </Button>
          )}
          <ThemeToggle />
